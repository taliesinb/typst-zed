...
`talimist serve --anno --open annotated-new.typ`, the `--open` flag will
...
with the content of the annotation living in the sidecar file (e.g. `annotated-new.annos.typ`,
...
- The sidecar `annotated-new.annos.typ` holds one `#metadata` entry per
  annotation; read it with `typst query annotated-new.annos.typ metadata`.
